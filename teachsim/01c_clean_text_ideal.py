...
ideal_dict = clean_text.import_text(
    start.RAW_FILEPATH + "models_v2/", pattern="behavior*.docx"
)
...
# %% Feedback
ideal_dict = clean_text.import_text(
    start.RAW_FILEPATH + "models_v2/", pattern="feedback*.docx"
)
df = pd.DataFrame.from_dict(ideal_dict, orient="index")
df = df.rename(columns={0: "clean_text"})
...
# %%
model_df.to_csv(start.CLEAN_FILEPATH + "text_scriptsV2.csv")
