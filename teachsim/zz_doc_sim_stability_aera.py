...
def similarity(df, matrix):
    for maindoc in list(matrix.index):
        ...
        average = (sum(pairwise_sim) - 1) / (
            len(pairwise_sim) - 1
        )  # don't include relationship with self
...
matrix_corpus_stop_stem_wgt = pd.read_csv(
    clean_filepath + "matrix_open_stop_stem_wgt.csv"
)
...
matrix_corpus_lsa = pd.DataFrame(
    lsa_fit, index=matrix_corpus.index, columns=word_weights.index
)
...
## Each document is a linear combination of components
matrix_corpus_wgt_lsa = pd.DataFrame(
    lsa_fit, index=matrix_corpus_wgt.index, columns=word_weights.index
)
...
## Each document is a linear combination of components
matrix_corpus_stop_wgt_lsa = pd.DataFrame(
    lsa_fit, index=matrix_corpus_stop_wgt.index, columns=word_weights.index
)
